...
class RewardedViewController: UIViewController {
    ...
    @IBAction private func didLoadAd(_ sender: Any) {
        ...
        DM.RewardedAd.load(placementKey: "a23c4147ab4d03ecdb937b15a3c3dc60") { [weak self] result in
            switch result {
            case .success(let ad):
                self?.show(ad: ad)
                break
        ...
        DM.shared.adService.load(nativeAd: "a23c4147ab4d03ecdb937b15a3c3dc60") { result in
            switch result {
            case .success(let ad):
                
                var kk: [String: Any]? = ad as? [String: Any]
